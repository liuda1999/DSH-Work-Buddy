...
def build_sections(pages: list[dict], slug_to_id: dict[str, str]) -> tuple[list[dict], list[dict]]:
    """复杂文档（≥2 个二级/三级标题）自动生成章节节点与 contains/part_of 边。

    让智能体生成上传的长文档在图谱中自带内部结构，而非孤立单点。
    """
    ...
    for p in pages:
        doc_id = slug_to_id.get(p["slug"])
        if not doc_id:
            continue
        headings = [m.group(1).strip() for m in HEADING_RE.finditer(p.get("body") or "")]
        if len(headings) < 2:
            continue  # 结构简单（无/仅 1 个标题）不生成章节
        for i, title in enumerate(headings, 1):
            sec_id = f"{doc_id}:sec:{i}"
            if sec_id in seen_nodes:
                continue
            seen_nodes.add(sec_id)
            section_nodes.append({
                ...
                "aliases": [],
                "path": p["rel_path"],
                "created": p["meta"].get("created") or "",
                "updated": p["meta"].get("updated") or "",
                "canonical": False,
            })
            push(make_edge(
                subject=doc_id, predicate="contains", obj=sec_id,
                source=None, evidence=None, confidence="high", status="current",
                extraction_method="heading_sections", page=p["rel_path"],
            ))
            push(make_edge(
                subject=sec_id, predicate="part_of", obj=doc_id,
                source=None, evidence=None, confidence="high", status="current",
                extraction_method="heading_sections", page=p["rel_path"],
            ))

    return section_nodes, section_edges
...
def main():
    ...
    sec_nodes, sec_edges = build_sections(pages, slug_to_id)
    nodes = nodes + sec_nodes
    edges = edges + sec_edges
