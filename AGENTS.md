...
> 项目版本：**v0.1.8**（见根目录 `VERSION`）。修改本项目时请先阅读本节与对应模块章节，遵循末尾「项目约定」。
...
- 对话组件直连 dsh：`session.models`（模型选择器，含「⟳ 刷新」）、`agentPreset.list`（工作模式，含**通用兼容**模式：面向非 DeepSeek 第三方/本地模型，装配 tool-skill/文件编辑/持久终端，本地文件系统无沙箱；第三方模型下自动/手动切换有权限护栏）、`session.history` 投影（权限级别 / 上下文卡 / 统计卡）。
